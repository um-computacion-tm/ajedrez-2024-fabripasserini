...
class Alfil(Pieza):
    ...
    def posiciones_validas(self, desde_fila, desde_columna, hasta_fila, hasta_columna):
        posible_posiciones = (
        self.posibles_posiciones_diagonal_sd(desde_fila, desde_columna) +
            self.posibles_posiciones_diagonal_si(desde_fila, desde_columna) +
            self.posibles_posiciones_diagonal_id(desde_fila, desde_columna) +
            self.posibles_posiciones_diagonal_ii(desde_fila, desde_columna)
        )
        return (hasta_fila, hasta_columna) in posible_posiciones

    def posibles_posiciones_diagonal_sd(self, fila, columna):
        posibles = []
    ...
    def posibles_posiciones_diagonal_si(self, fila, columna):
        posibles = []
        siguiente_fila, siguiente_columna = fila + 1, columna - 1
